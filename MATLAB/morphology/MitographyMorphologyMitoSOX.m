%%%
% Mitography - morphology and MitoSOX
...
for fileNum = fileNumbers
    ...
    filepathpxs = strFilepath(fileNum,filenameallPxs,masterFolderPath);
    filepathMitoSOX = strFilepath(fileNum,filenameMitoSOX,masterFolderPath);
...
%         disp('mito small done')
        ...
        %%% MITOCHONDRIA MITOSOX CHECK AND FLAGGING
        if ~isempty(dataAnalysis)
            for i=1:sizeData(1)
                ...
                dataAnalysis(i,111) = mitosoxsignalavg;  % MITOSOX SIGNAL
                dataAnalysis(i,115) = ompsignalavg;  % OMP25 SIGNAL
            end

            %decide what is the limiting mitoSOX signal for pos/neg
            ...
            mitosoxvals = dataAnalysis(:,111);
            mitosoxvals = mitosoxvals(~isnan(mitosoxvals));
            mitosoxvals = sort(mitosoxvals);
            % remove the biggest values, as they might be outliers, and
            % this will not affect the results if they are not
%             mitosoxvals = mitosoxvals(1:end-round(sizeData(1)/10));
            % decide the binwidths and get bins from 0, otherwise the bins
            % start from min(mitosoxvals), which might be high above 0, meaning
            % that the fit would not know what to fit around 0. If there is
            % no values close to 0, there is probably no non-signal mitos!
            binwid = (max(mitosoxvals)-min(mitosoxvals))/numgroups;
            binedges = 0:binwid:max(mitosoxvals)+binwid;
            [cnts,edges] = histcounts(mitosoxvals,numgroups,'Normalization','probability');
            edges(end) = [];
            ...
            opts.Display = 'Off';
            opts.Lower = [0 0 0.3 median(mitosoxvals)*1/4 median(mitosoxvals)*1/4];
            opts.StartPoint = [0.2 0.2 1 median(mitosoxvals) median(mitosoxvals)*1/2];
            opts.Upper = [0.5 0.5 3 max(mitosoxvals)*0.9 median(mitosoxvals)*3/2];
            ...
            stepsize = x(end)/max(2*max(mitosoxvals), 100);
            ...
            for i=1:sizeData(1)
                ...
                if mitosoxsignal > threshsignal
                    dataAnalysis(i,112) = 1;  % MITOSOX PARAM
                else
