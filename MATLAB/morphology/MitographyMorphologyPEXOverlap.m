...
for fileNum = fileNumbers
    ...
    filepathPEXBinary = strFilepath(fileNum,filenamePEXBinary,masterFolderPath);
    ...
    try
        ...
        try
            ...
            imagepexbinary = zeros(size(imagemitobinary));
            imagepexbinary = logical(imagepexbinary);
        end
...
%         disp('mito small done')
        ...
        %%% MITOCHONDRIA PEX BINARY OVERLAP CHECK
        if not(isempty(dataAnalysis))
            for i=1:sizeData(1)
                % get binary img of single mitochondria
                singlemitobinary = ismember(labelmito, i);
                % check if any pex is overlapping with this mito
                pixelsum = logical(sum(sum(imagepexbinary.*singlemitobinary)));
                % save boolean variable for which mito has PEX overlap
                if pixelsum
