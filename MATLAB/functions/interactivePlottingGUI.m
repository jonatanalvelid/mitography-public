...
% Create two figures
h.f = figure('units','pixels','position',[1400 100 200 800],...
             'toolbar','none','menu','none');
h.fplot = figure('units','pixels','position',[20 100 1300 700],...
...
%             
    ...
    function p10_call(varargin)
        ...
        [filamentWidthSNZ, mitoDisplacementEdgeS, mitoActinDistS, mitoActinDistD2S, filamentWidthNSNZ, mitoDisplacementEdgeNS,...
            ...
            mitoWidthRatioBMNS, mitoWidthRatioUBNS, mitoPatchOverlapS, mitoPatchOverlapNS, filamentWidthPNZ, mitoDisplacementEdgeP, mitoActinDistP, mitoActinDistD2P,...
            filamentWidthNPNZ, mitoDisplacementEdgeNP, mitoActinDistNP, mitoActinDistD2NP, mitoWidthP, mitoAreaP, mitoLengthP, nonzeroP, mitoWidthNP, mitoAreaNP, mitoLengthNP,...
            nonzeroNP, mitoPatchOverlapP, mitoPatchOverlapNP, mitoNonStripesDistP, mitoNonStripesDistNP, paramName] = updateData(doubleThreshold, doubleSecondThreshold, singleThreshold);
    end
    ...
    function p1_call(varargin)
        ...
        if paramPlots
            ...
            subplot(1,4,1)
            newDoubleHistogram(filamentWidthPNZ,filamentWidthNPNZ,0,0.07,1.4,0,1.5,0.4,'Local filament width [um]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            legend(paramName,strcat('Non-',paramName));
            
            subplot(1,4,2)
            newDoubleHistogram(mitoDisplacementEdgeP,mitoDisplacementEdgeNP,-0.8,0.08,0.8,-0.8,0.8,0.4,'Old mito-actin distance [um]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            legend(paramName,strcat('Non-',paramName));
            
            subplot(1,4,3)
            newDoubleHistogram(mitoActinDistP,mitoActinDistNP,-1,0.08,0.7,-1,0.7,0.4,'Mito-actin distance D [um]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            legend(paramName,strcat('Non-',paramName));
            
            subplot(1,4,4)
            newDoubleHistogram(mitoActinDistD2P,mitoActinDistD2NP,0,0.04,1,0,1,0.4,'Mito-actin distance D2 [um]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            legend(paramName,strcat('Non-',paramName));
        else
            figure(h.fplot)
            clf(h.fplot)
            %%% Mito displacement and filament width histogram
            subplot(1,4,1)
            newDoubleHistogram(filamentWidthSNZ,filamentWidthNSNZ,0,0.07,1.4,0,1.5,0.4,'Local filament width [um]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
            legend('MPS','Non-MPS');
            
            subplot(1,4,2)
            newDoubleHistogram(mitoDisplacementEdgeS,mitoDisplacementEdgeNS,-0.8,0.08,0.8,-0.8,0.8,0.4,'Old mito-actin distance [um]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
            legend('MPS','Non-MPS');
            
            subplot(1,4,3)
            newDoubleHistogram(mitoActinDistS,mitoActinDistNS,-1,0.08,0.7,-1,0.7,0.4,'Mito-actin distance D [um]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
            legend('MPS','Non-MPS');
            
            subplot(1,4,4)
            newDoubleHistogram(mitoActinDistD2S,mitoActinDistD2NS,0,0.04,1,0,1,0.4,'Mito-actin distance D2 [um]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
            legend('MPS','Non-MPS');
        end
    end
    ...
    function p2_call(varargin)
        ...
        if paramPlots
            ...
            subplot(2,3,1)
            newScatter(filamentWidthPNZ,mitoWidthP(nonzeroP),0,1.5,0,0.5,'Local filament width [um]','Mitochondria width [um]',paramName,fontSizeGlobal);
            newScatter(filamentWidthNPNZ,mitoWidthNP(nonzeroNP),0,1.5,0,0.5,'Local filament width [um]','Mitochondria width [um]',strcat('Non-',paramName),fontSizeGlobal);
            legend(paramName,strcat('Non-',paramName));
            subplot(2,3,2)
            newScatter(filamentWidthPNZ,mitoAreaP(nonzeroP),0,1.5,0,1,'Local filament width [um]','Mitochondria area [um^2]',paramName,fontSizeGlobal);
            newScatter(filamentWidthNPNZ,mitoAreaNP(nonzeroNP),0,1.5,0,1,'Local filament width [um]','Mitochondria area [um^2]',strcat('Non-',paramName),fontSizeGlobal);
            legend(paramName,strcat('Non-',paramName));
            subplot(2,3,3)
            newScatter(filamentWidthPNZ,mitoLengthP(nonzeroP),0,1.5,0,5,'Local filament width [um]','Mitochondria length [um]',paramName,fontSizeGlobal);
            newScatter(filamentWidthNPNZ,mitoLengthNP(nonzeroNP),0,1.5,0,5,'Local filament width [um]','Mitochondria length [um]',strcat('Non-',paramName),fontSizeGlobal);
            legend(paramName,strcat('Non-',paramName));
            subplot(2,3,4)
            newScatter(filamentWidthNPNZ,mitoWidthNP(nonzeroNP),0,1.5,0,0.5,'Local filament width [um]','Mitochondria width [um]',strcat('Non-',paramName),fontSizeGlobal);
            subplot(2,3,5)
            ...
            %%% Mito width, length and area vs filament width plotting
            subplot(2,3,1)
            newScatter(filamentWidthSNZ,mitoWidthS(nonzeroS),0,1.5,0,0.5,'Local filament width [um]','Mitochondria width [um]','Stripes',fontSizeGlobal);
            newScatter(filamentWidthNSNZ,mitoWidthNS(nonzeroNS),0,1.5,0,0.5,'Local filament width [um]','Mitochondria width [um]','Non-stripes',fontSizeGlobal);
            legend('MPS','Non-MPS');
            subplot(2,3,2)
            newScatter(filamentWidthSNZ,mitoAreaS(nonzeroS),0,1.5,0,1,'Local filament width [um]','Mitochondria area [um^2]','Stripes',fontSizeGlobal);
            newScatter(filamentWidthNSNZ,mitoAreaNS(nonzeroNS),0,1.5,0,1,'Local filament width [um]','Mitochondria area [um^2]','Non-stripes',fontSizeGlobal);
            legend('MPS','Non-MPS');
            subplot(2,3,3)
            newScatter(filamentWidthSNZ,mitoLengthS(nonzeroS),0,1.5,0,5,'Local filament width [um]','Mitochondria length [um]','Stripes',fontSizeGlobal);
            newScatter(filamentWidthNSNZ,mitoLengthNS(nonzeroNS),0,1.5,0,5,'Local filament width [um]','Mitochondria length [um]','Non-stripes',fontSizeGlobal);
            legend('MPS','Non-MPS');
            subplot(2,3,4)
            newScatter(filamentWidthNSNZ,mitoWidthNS(nonzeroNS),0,1.5,0,0.5,'Local filament width [um]','Mitochondria width [um]','Non-stripes',fontSizeGlobal);
            subplot(2,3,5)
    ...
    function p3_call(varargin)
        ...
        if paramPlots
            ...
            subplot(1,3,1)    
            newDoubleHistogram(mitoWidthP,mitoWidthNP,0,0.015,1,0,0.6,0.2,'Mitochondria width [um]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            %newDoubleHistFit(mitoWidthP,mitoWidthNP,0,0.015,1)
            legend(paramName,strcat('Non-',paramName));
            
            subplot(1,3,2)
            newDoubleHistogram(mitoAreaP,mitoAreaNP,0,0.05,20,0,2,0.2,'Mitochondria area [um^2]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            %newDoubleHistFit(mitoAreaP,mitoAreaNP,0,0.05,20)
            legend(paramName,strcat('Non-',paramName));
            
            subplot(1,3,3)
            newDoubleHistogram(mitoLengthP,mitoLengthNP,0,0.1,20,0,4,0.2,'Mitochondria length [um]','',fontSizeGlobal,paramName,strcat('Non-',paramName))
            ...
            %%% Mito width, length and area plotting
            subplot(1,3,1) 
            newDoubleHistogram(mitoWidthS,mitoWidthNS,0,0.015,1,0,0.6,0.2,'Mitochondria width [um]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
            %newDoubleHistFit(mitoWidthS,mitoWidthNS,0,0.015,1)
            legend('MPS','Non-MPS');
            
            subplot(1,3,2)
            newDoubleHistogram(mitoAreaS,mitoAreaNS,0,0.05,20,0,2,0.2,'Mitochondria area [um^2]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
            %newDoubleHistFit(mitoAreaS,mitoAreaNS,0,0.05,20)
            legend('MPS','Non-MPS');
            
            subplot(1,3,3)
            newDoubleHistogram(mitoLengthS,mitoLengthNS,0,0.1,20,0,4,0.2,'Mitochondria length [um]','',fontSizeGlobal,'MPS',strcat('Non-','MPS'))
    ...
    function p4_call(varargin)
        ...
        if paramPlots
            ...
            subplot(2,3,1)
            newScatter(mitoWidthP,mitoLengthP,0,0.5,0,5,'Mitochondria width [um]','Mitochondria length [um]',paramName,fontSizeGlobal);
            newScatter(mitoWidthNP,mitoLengthNP,0,0.5,0,5,'Mitochondria width [um]','Mitochondria length [um]',strcat('Non-',paramName),fontSizeGlobal);
            legend(paramName,strcat('Non-',paramName));
            subplot(2,3,2)
            ...
            newScatter(filamentWidthPNZ,yvar(nonzeroP),0,1.5,0,15,'Local filament width [um]','Mitochondria length/mitochondria width [arb.u.]',paramName,fontSizeGlobal);
            yvar = mitoLengthNP./mitoWidthNP;
            newScatter(filamentWidthNPNZ,yvar(nonzeroNP),0,1.5,0,15,'Local filament width [um]','Mitochondria length/mitochondria width [arb.u.]',strcat('Non-',paramName),fontSizeGlobal);
            legend(paramName,strcat('Non-',paramName));
            subplot(2,3,4)
            newScatter(mitoWidthNP,mitoLengthNP,0,0.5,0,5,'Mitochondria width [um]','Mitochondria length [um]',strcat('Non-',paramName),fontSizeGlobal);
            subplot(2,3,5)
            ...
            %%% Mito length vs mito width plotting
            subplot(2,3,1)
            newScatter(mitoWidthS,mitoLengthS,0,0.5,0,5,'Mitochondria width [um]','Mitochondria length [um]','Stripes',fontSizeGlobal);
            newScatter(mitoWidthNS,mitoLengthNS,0,0.5,0,5,'Mitochondria width [um]','Mitochondria length [um]','Non-stripes',fontSizeGlobal);
            legend('MPS','Non-MPS');
            subplot(2,3,2)
            ...
            newScatter(filamentWidthSNZ,yvar(nonzeroS),0,1.5,0,15,'Local filament width [um]','Mitochondria length/mitochondria width [arb.u.]','Stripes',fontSizeGlobal);
            yvar = mitoLengthNS./mitoWidthNS;
            newScatter(filamentWidthNSNZ,yvar(nonzeroNS),0,1.5,0,15,'Local filament width [um]','Mitochondria length/mitochondria width [arb.u.]','Non-stripes',fontSizeGlobal);
            legend('MPS','Non-MPS');
            subplot(2,3,4)
            newScatter(mitoWidthNS,mitoLengthNS,0,0.5,0,5,'Mitochondria width [um]','Mitochondria length [um]','Non-stripes',fontSizeGlobal);
            subplot(2,3,5)
    ...
    function p5_call(varargin)
        ...
        if paramPlots
            ...
            subplot(1,3,1)
            newHistogram(mitoNonStripesDistP,0,0.1,15,0,7,0.3,'Distance to closest actin patch [um]','',fontSizeGlobal);
            newHistogram(mitoNonStripesDistNP,0,0.1,15,0,7,0.3,'Distance to closest actin patch [um]','',fontSizeGlobal);
            legend(paramName,strcat('Non-',paramName));
            subplot(1,3,2)
            newHistogram([mitoNonStripesDistP;mitoNonStripesDistNP],0,0.1,15,0,7,0.15,'Distance to closest actin patch [um]','All mito',fontSizeGlobal);
        else
            figure(h.fplot)
            clf(h.fplot)
            %%% Mito non-stripes distance histogram - three peaks mito vs all other mito
            subplot(1,3,1)
            newHistogram(mitoNonStripesDistS,0,0.1,15,0,7,0.3,'Distance to closest actin patch [um]','',fontSizeGlobal);
            newHistogram(mitoNonStripesDistNS,0,0.1,15,0,7,0.3,'Distance to closest actin patch [um]','',fontSizeGlobal);
            legend('MPS','Non-MPS');
            subplot(1,3,2)
            newHistogram([mitoNonStripesDistS;mitoNonStripesDistNS],0,0.1,15,0,7,0.15,'Distance to closest actin patch [um]','All mito',fontSizeGlobal);
            subplot(1,3,3)
            newHistogram(mitoThreePeaksNonStripesDist,0,0.1,15,0,7,0.15,'Distance to closest actin patch [um]','Three-peaks mito',fontSizeGlobal);
        end
    end
    ...
    function p6_call(varargin)
        ...
        if paramPlots
            ...
            subplot(1,3,1)
            newHistogram([mitoWidthS;mitoWidthNS],0,0.015,1,0,0.6,0.2,'Mitochondria width [um]','All mito',fontSizeGlobal);
            subplot(1,3,2)
            newHistogram([mitoAreaS;mitoAreaNS],0,0.05,20,0,2,0.2,'Mitochondria area [um^2]','All mito',fontSizeGlobal);
            subplot(1,3,3)
            newHistogram([mitoLengthS;mitoLengthNS],0,0.1,4,0,4,0.2,'Mitochondria length [um]','All mito',fontSizeGlobal);
        end
    end
