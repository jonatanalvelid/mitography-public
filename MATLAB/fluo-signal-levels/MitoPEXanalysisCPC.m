%%%
% PEX signal levels - analysis and cell per cell plotting
%
% @jonatanalvelid
%%%
...
% Data folder
if example_data
    masterFolderPath = fullfile(datafolder,'fluo-signal-levels','pex\');
else
...
for i=1:max(expnum)
    subplot(max(expnum),max(expnum)/2,subplotpos(i));
    hold on
...
% Box plots with jittered scatter of PEX+ mean areas
figure()
hold on
labels = {'Exp1 - AA', 'Exp1 - CTR'};
...
% T-tests for PEX- and PEX+ mean areas between datasets
disp(' ')
for n=1:max(expnum)
    for m=1:max(expnum)
        if n ~= m && m>n
